...
objects_to_link = ""
bin_path = game_path+"/bin/"+project_platform+"/"+project_configuration
library_path = VSSE+"/bin/x"+project_platform+"/"+project_configuration
library_includes = library_path+"/Core.lib "+library_path+"/Rendering.lib "+ library_path+"/Input.lib "+library_path+"/Transform.lib "+ library_path+"/Audio.lib "+library_path+"/Physics.lib "

for i in range(6,len(sys.argv)):
    objects_to_link += game_path+"/Intermediate/Objects/"+project_configuration+"/"+project_platform+"/"+sys.argv[i] + ".obj "

if project_configuration == "Release":
	configuration_dependant = " /DEBUG:NONE"
else:
	configuration_dependant = " /DEBUG /PDB:"+game_path+"\\Intermediate\\Scripts"+DLL_ID+".pdb /PDBALTPATH:"+game_path+"\\Intermediate\\Scripts"+DLL_ID+".pdb"
